...
namespace AMK.Models.ViewModels
{
    public partial class UsersViewModel : ObservableObject
    {
        ...
        private async Task SaveUserAsync()
        {
            ...
            if (string.IsNullOrWhiteSpace(SelectedUser?.FIO) ) errors.Add("ФИО - обязательное поле");
            if (string.IsNullOrWhiteSpace(SelectedUser?.Login) && !Validators.IsValidLogin(SelectedUser?.Login)) errors.Add("Логин - обязательное поле");
            if (string.IsNullOrWhiteSpace(SelectedUser?.Password) && !Validators.IsValidPassword(SelectedUser?.Password)) errors.Add("Пароль - обязательное поле");

            if (!string.IsNullOrWhiteSpace(SelectedUser?.Email) && !Validators.IsValidEmail(SelectedUser.Email))
                errors.Add("Некорректный Email");

            if (!string.IsNullOrWhiteSpace(SelectedUser?.Phone) && !Validators.IsValidPhone(SelectedUser.Phone))
