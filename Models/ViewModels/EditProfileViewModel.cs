...
namespace AMK.Models.ViewModels
{
    public partial class EditProfileViewModel : ObservableObject
    {
        ...
        private async Task SaveAsync()
        {
            ...
            if (string.IsNullOrWhiteSpace(EditableUser.FIO)) errors.Add("ФИО - обязательное поле");
            if (!string.IsNullOrWhiteSpace(EditableUser.Email) && !Validators.IsValidEmail(EditableUser.Email)) errors.Add("Некорректный Email");
            if (!string.IsNullOrWhiteSpace(EditableUser.Phone) && !Validators.IsValidPhone(EditableUser.Phone)) errors.Add("Некорректный телефон");
